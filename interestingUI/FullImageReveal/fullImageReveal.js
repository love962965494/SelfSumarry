...
let winsize = { wioth: window.innerWidth, height: window.innerHeight };
let allowTilt = true;
...
class GridItem {
  ...
  toggle(action) {
        ...
        y: () => {
          if (action === 'hide') {
            if (this.constructor.name === 'Thumb') {
              return -1 * winsize.height - 30;
            }
            return -1 * winsize.height - 30 + inner.offsetHeight / 2;
          }
          return 0;
        },
      });
    ...
    if (this.constructor.name === 'GridItem') {
      // eslint-disable-next-line
      TweenMax.to(
        this.DOM.el.querySelector('.grid__toggle-more'),
        action === 'hide' ? 0.2 : 0.4, {
      ...
      // eslint-disable-next-line
      TweenMax.to(
        this.DOM.el.querySelector('.grid__toggle-back'),
        action === 'hide' ? 0.4 : 0.2, {
...
class Thumb extends GridItem {
  ...
  tilt(ev) {
    ...
    Object.keys(this.DOM.tilt).forEach((key) => {
      const t = this.tileConfig[key].translation;
